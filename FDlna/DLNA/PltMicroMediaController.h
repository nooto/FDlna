...
public:
    ...
    PLT_MicroMediaController(PLT_CtrlPointReference& ctrlPoint,SHDMRControl * delegateWrapper);
...
//                         void*                    /* userdata */) ;
    ...
    NPT_String buildLocalResourceUri(MPMediaItem *theRes) ;
    
    NPT_String buildSafeResourceUri(
                           const NPT_HttpUrl&base_uri               /* res */,
                           const char* host /* device */,
                            const char* theRes) ;
...
public:
    ...
    BOOL    chooseMeidaServer(NPT_String chosenUUID);
    BOOL    chooseMediaRenderer(NPT_String chosenUUID);
    
    PLT_DeviceDataReference getCurrentMediaServer();
    PLT_DeviceDataReference getCurrentMediaRenderer();
    
    NPT_Result    setRendererMute();
    NPT_Result    getRendererMute();
    NPT_Result    setRendererUnMute();
    NPT_Result    setRendererVolume(int volume, void *userdata);
    NPT_Result    getRendererVolume();
    NPT_Result    sendSeekCommand(const char* command,  void* userdata);
    
    NPT_Result    setRendererPlay();
    NPT_Result    setRendererPause();
    NPT_Result    setRendererStop();
    NPT_Result    setRendererNext();
    NPT_Result    setRendererPrevious();
    NPT_Result    setRendererAVTransportURI(const char *uriStr, const char *metaData, void* userdata);
    bool          canRendererSetNextURI();
    NPT_Result    setRendererNextAVTransportURI(const char *nextUriStr);
    
    NPT_Result    setRendererPlayMode(const char *new_play_mode);
    NPT_Result    getRendererCurrentTransportActions();
    NPT_Result    getRendererDeviceCapabilities();
    NPT_Result    getRendererProtocolInfo();
    
    NPT_Result    getRendererMediaInfo();
    NPT_Result    getRendererPositionInfo();
    NPT_Result    getRendererTransportInfo();
    NPT_Result    getRendererTransportSettings();
...
private:
    ...
    NPT_List<NPT_String> browse(const char* object_id = NULL, bool metdata = false);
