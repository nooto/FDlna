...
/*----------------------------------------------------------------------
 ...
 +---------------------------------------------------------------------*/
PLT_MicroMediaController::PLT_MicroMediaController(PLT_CtrlPointReference& ctrlPoint,SHDMRControl * delegateWrapper) :
...
PLT_MicroMediaController::OnMSAdded(PLT_DeviceDataReference& device)
{
    ...
    if (NPT_SUCCEEDED(device->FindServiceByType("urn:microsoft.com:service:X_MS_MediaReceiverRegistrar:*", service))) {
        ...
        NPT_String urlString = device->GetDescriptionUrl();
        
        if(m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(OnMSAdded)])
        {
            [m_Target.delegate OnMSAdded];
        }
        NSLog(@"plt_____OnMSAdded----");
...
PLT_MicroMediaController::OnMSRemoved(PLT_DeviceDataReference& device)
{
    ...
    if(m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(OnMSRemoved:)])
    {
        [m_Target.delegate OnMSRemoved:[NSString stringWithUTF8String:device->GetUUID()]];
    }
}
...
PLT_MicroMediaController::OnMRRemoved(PLT_DeviceDataReference& device)
{
    ...
    {
        [m_Target.delegate onDMRRemoved:[NSString stringWithUTF8String:device->GetUUID()]];
    }
}
...
{
    NSMutableArray<SHEventParamsResponse *> *stateArray = [NSMutableArray array];
...
//#endif
        ...
        SHEventParamsResponse * state = [[SHEventParamsResponse alloc] initWithDeviceUUID:deviceUUID ServiceID:serviceID EventName:eventName EventValue:eventValue];
...
PLT_MicroMediaController::chooseMeidaServer(NPT_String chosenUUID)
{
    ...
    return m_CurMediaServer.IsNull();
}
...
PLT_MicroMediaController::chooseMediaRenderer(NPT_String chosenUUID)
{
    ...
    return m_CurMediaRenderer.IsNull();
}
...
NPT_List<NPT_String> PLT_MicroMediaController::browse( const char* obj_id, bool metadata)
{
    //eid`
    NPT_List<NPT_String>  mediadidllist;
    DoBrowse(obj_id, metadata);
    if (!m_MostRecentBrowseResults.IsNull()) {
        // create a map item id -> item title
        NPT_List<PLT_MediaObject*>::Iterator item = m_MostRecentBrowseResults->GetFirstItem();
        while (item) {
            //             if (!(*item)->IsContainer()) {
            //                 LOGE("ObjectID:%s, title:%s", (*item)->m_ObjectID.GetChars(), (*item)->m_Title.GetChars());
            //             }
            //            LOGE("ObjectID:%s, title:%s", (*item)->m_ObjectID.GetChars(), (*item)->m_Title.GetChars());
            mediadidllist.Add((*item)->m_Didl);
            ++item;
        }
        m_MostRecentBrowseResults = NULL;
    }
    return mediadidllist;
...
NPT_Result
PLT_MicroMediaController::setRendererAVTransportURI(const char *uriStr, const char *metaData,void* userdata)
{
    PLT_DeviceDataReference device;
    GetCurMediaRenderer(device);
    NPT_Result reslut = NPT_FAILURE;
    if (!device.IsNull()) {
        ...
        //printf("Issuing SetAVTransportURI with url=%s & didl=%s",
          //     uriStr,
            //   (const char*)"");
        reslut = (SetAVTransportURI(device, 0, uriStr, metaData == NULL ?"" : metaData, userdata));
    }
    return reslut;
}
...
//               nextUriStr,
//               (const char*)"");
        reslut =SetNextAVTransportURI(device, 0, nextUriStr, "", NULL);
...
PLT_MicroMediaController::setRendererPlayMode(const char *new_play_mode)
{
    ...
    if (!device.IsNull()) {
        reslut = SetPlayMode(device, 0, NPT_String(new_play_mode), NULL);
    }
    return reslut;
...
PLT_MicroMediaController::getRendererCurrentTransportActions()
{
    ...
    if (!device.IsNull()) {
       reslut = GetCurrentTransportActions(device, 0, NULL);
    }
    return reslut;
...
PLT_MicroMediaController::getRendererDeviceCapabilities()
{
    ...
    if (!device.IsNull()) {
       reslut = GetDeviceCapabilities(device, 0, NULL);
    }
    return reslut;
}
...
PLT_MicroMediaController::getRendererProtocolInfo()
{
    ...
    if (!device.IsNull()) {
       reslut = GetProtocolInfo(device, NULL);
    }
    return reslut;
}
...
PLT_MicroMediaController::getRendererMediaInfo()
{
    ...
    if (!device.IsNull()) {
       reslut = GetMediaInfo(device, 0, NULL);
    }
    return reslut;
}
...
PLT_MicroMediaController::getRendererPositionInfo()
{
    ...
    if (!device.IsNull()) {
       reslut = GetPositionInfo(device, 0, NULL);
    }
    return reslut;
}
...
PLT_MicroMediaController::getRendererTransportInfo()
{
    ...
    if (!device.IsNull()) {
        reslut =  GetTransportInfo(device, 0, NULL);
    }
    return reslut;
}
...
PLT_MicroMediaController::getRendererTransportSettings()
{
    ...
    if (!device.IsNull()) {
       reslut = GetTransportSettings(device, 0, NULL);
    }
    return reslut;
}
...
/*----------------------------------------------------------------------
 ...
 +---------------------------------------------------------------------*/
NPT_Result
PLT_MicroMediaController::sendSeekCommand(const char* command, void* userdata)
{
    PLT_DeviceDataReference device;
    GetCurMediaRenderer(device);
    NPT_Result reslut = NPT_FAILURE;
    ...
    if (!device.IsNull()) {
       ...
       reslut =  Seek(device, 0, (target.Find(":")!=-1)?"REL_TIME":"X_DLNA_REL_BYTE", target, userdata);
    }
    return reslut;
}
...
PLT_MicroMediaController::setRendererMute()
{
    ...
    if (!device.IsNull()) {
       reslut = SetMute(device, 0, "Master", false, NULL);
    }
    return reslut;
}
...
PLT_MicroMediaController::getRendererMute()
{
    ...
    if (!device.IsNull()) {
       reslut = GetMute(device, 0, "Master", NULL);
    }
    return reslut;
}
...
PLT_MicroMediaController::setRendererUnMute()
{
    ...
    if (!device.IsNull()) {
         reslut = SetMute(device, 0, "Master", true, NULL);
    }
    return reslut;
}
/*----------------------------------------------------------------------
 ...
 +---------------------------------------------------------------------*/
NPT_Result
PLT_MicroMediaController::setRendererVolume(int volume, void* userdata)
{
    PLT_DeviceDataReference device;
    GetCurMediaRenderer(device);
    if (!device.IsNull()) {
        return SetVolume(device, 0, "Master", volume, userdata);
    }
    return NPT_FAILURE;
}
...
PLT_MicroMediaController::getRendererVolume()
{
    ...
    GetCurMediaRenderer(device);
    if (!device.IsNull()) {
        return  GetVolume(device, 0, "Master", NULL);
    }
    return NPT_FAILURE;
}
...
//        namesTable.Put((*entry)->GetKey(), name);
        //修改为name+ip 地址
        NPT_String              GetLocalIP   = device->GetLocalIP().ToString();
        namesTable.Put((*entry)->GetKey(), name + "#" + GetLocalIP);
                                   ...
                                   void*                    userdata)
{
    ...
    if(!device.IsNull() &&  m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(getCurrentAVTransportActionResponse:)])
    {
        ...
        SHCurrentAVTransportActionResponse *capsule = [[SHCurrentAVTransportActionResponse alloc]initWithResult:result DeviceUUID:deviceUUID Actions:actionArr UserData: (__bridge id)userdata];
                              ...
                              void*                    userdata)
{
    if(m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(OnGetDeviceCapabilitiesResult:)])
    {
        ...
        PLT_StringList::Iterator iter2 = capabilities->rec_media.GetFirstItem();
        while (iter2) {
            if ((*iter2).GetChars()) {
                NSString *capbility = [NSString stringWithUTF8String:(*iter2).GetChars()];
                [capabilitiesArr addObject:capbility];
                iter2++;
            }
            else{
        ...
        PLT_StringList::Iterator iter3 = capabilities->rec_quality_modes.GetFirstItem();
        while (iter3) {
            NSString *capbility = [NSString stringWithUTF8String:(*iter3).GetChars()];
            [capabilitiesArr addObject:capbility];
            iter3++;
        }
        ...
        SHDeviceCapabilitiesResponse *capsule = [[SHDeviceCapabilitiesResponse alloc]initWithResult:result DeviceUUID:deviceUUID UserData: (__bridge id)userdata];
        capsule.capabilitiesArr = capabilitiesArr;
                     ...
                     void*                    userdata)
{
    ...
    if(!device.IsNull() &&  info != NULL && m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(OnGetMediaInfoResult:)])
    {
        NSInteger result = res;
        NSString *deviceUUID = [NSString stringWithUTF8String:device->GetUUID()];
        
        SHMediaInfoResponse *capsule = [[SHMediaInfoResponse alloc]initWithResult:result DeviceUUID:deviceUUID UserData: (__bridge id)userdata];
        capsule.num_tracks = info->num_tracks;
        capsule.media_duration = info->media_duration;
...
OnGetPositionInfoResult(
                        ...
                        void*                    userdata)
{
    if(!device.IsNull() &&  info != NULL && m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(getPositionInfoResponse:)])
    {
        NSInteger result = res;
        NSString *deviceUUID = [NSString stringWithUTF8String:device->GetUUID()];
        if (info != NULL) {
            SHPositionInfoResponse *capsule = [[SHPositionInfoResponse alloc]initWithResult:result DeviceUUID:deviceUUID UserData: (__bridge id)userdata];
            capsule.track = info->track;
            capsule.track_duration = info->track_duration.ToSeconds();
            ...
            capsule.rel_time = info->rel_time.ToSeconds();
            capsule.abs_time = info->abs_time.ToSeconds();
            capsule.rel_count = info->rel_count;
            capsule.abs_count = info->abs_count;
...
OnGetTransportInfoResult(
                         ...
                         void*                    userdata)
{
    if(!device.IsNull() &&  info != NULL && m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(getTransportInfoResponse:)])
    {
        NSInteger result = res;
        NSString *deviceUUID = [NSString stringWithUTF8String:device->GetUUID()];
        
        SHTransportInfoResponse *capsule = [[SHTransportInfoResponse alloc]initWithResult:result DeviceUUID:deviceUUID UserData: (__bridge id)userdata];
...
OnNextResult(
             ...
             void*                    userdata)
{
    if(!device.IsNull() && m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(nextResponse:)])
    {
...
OnPauseResult(
              ...
              void*                    userdata)
{
    if(!device.IsNull() && m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(pasuseResponse:)])
    {
        NSInteger result = res;
        NSString *deviceUUID = [NSString stringWithUTF8String:device->GetUUID()];
...
OnPlayResult(
             ...
             void*                    userdata)
{
    if(!device.IsNull() &&  m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(playResponse:)])
    {
        NSInteger result = res;
        NSString *deviceUUID = [NSString stringWithUTF8String:device->GetUUID()];
...
OnPreviousResult(
                 ...
                 void*                    userdata)
{
    if(!device.IsNull() &&  m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(previousResponse:)])
    {
        NSInteger result = res;
        NSString *deviceUUID = [NSString stringWithUTF8String:device->GetUUID()];
...
OnSeekResult(
             ...
             void*                    userdata)
{
    if(!device.IsNull() &&  m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(OnSeekResult:)])
    {
        NSInteger result = res;
        NSString *deviceUUID = [NSString stringWithUTF8String:device->GetUUID()];
...
OnSetAVTransportURIResult(
                          ...
                          void*                    userdata)
{
    if(!device.IsNull() &&  m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(setAVTransponrtResponse:)])
    {
        NSInteger result = res;
        NSString *deviceUUID = [NSString stringWithUTF8String:device->GetUUID()];
...
OnStopResult(
             ...
             void*                    userdata)
{
    if(!device.IsNull() &&  m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(stopResponse:)])
    {
        NSInteger result = res;
        NSString *deviceUUID = [NSString stringWithUTF8String:device->GetUUID()];
...
OnSetMuteResult(
                ...
                void*                    userdata)
{
    if(!device.IsNull() &&  m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(setMuteResponse:)])
    {
        NSInteger result = res;
        NSString *deviceUUID = [NSString stringWithUTF8String:device->GetUUID()];
        ...
        [m_Target.delegate setMuteResponse:capsule];
    }
}
//*/
/*
...
OnSetVolumeResult(
                  ...
                  void*                    userdata)
{
    if(!device.IsNull() &&  m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(setVolumeResponse:)])
    {
        NSInteger result = res;
        NSString *deviceUUID = [NSString stringWithUTF8String:device->GetUUID()];
...
OnGetVolumeResult(
                  ...
                  void*                    userdata)
{
    if(!device.IsNull() &&  m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(getVolumeResponse:)])
    {
        NSInteger result = res;
        NSString *deviceUUID = [NSString stringWithUTF8String:device->GetUUID()];
        ...
        SHGetVolumResponse * capsule = [[SHGetVolumResponse alloc] initWithResult:result DeviceUUID:deviceUUID UserData:(__bridge id)userdata Channel:cnl Volume:volume];
...
NPT_String PLT_MicroMediaController::buildLocalResourceUri(MPMediaItem *song)
{
    PLT_HttpRequestContext *context = nil;
...
//    MPMediaItem *song;
    ...
    while (ip) {
        resource.m_Uri = PLT_MicroMediaController::buildSafeResourceUri(base_uri, ip->ToString(), [[assetURL absoluteString] cStringUsingEncoding:NSUTF8StringEncoding]);
//        resource.m_Uri = [p buildSafeResourceUri:base_uri withHost:ip->ToString() withResource:[[assetURL absoluteString] cStringUsingEncoding:NSUTF8StringEncoding]];
...
NPT_String PLT_MicroMediaController::
            buildSafeResourceUri(const NPT_HttpUrl&base_uri,
                                 const char* host,
                                 const char* theRes)
{
    NPT_String result;
